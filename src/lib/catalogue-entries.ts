...
export type PluginEntry = CatalogueEntry & {
  /** From the local overlay, not the manifest. */
  audiences: Audience[];
...
export type SkillEntry = CatalogueEntry & {
  /** From the local overlay, not the generator. */
  audiences: Audience[];
  path: string;
  plugin: string;
  origin: string;
 ...
 *
 *  Category is deliberately absent: it is the section axis on `/skills`, the way a tool's category
 *  is on `/tools`, and a control that only picks the heading a card already sits under narrows
